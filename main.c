...
int main(void) {
	...
	for (size_t i = 0; i < win->w * win->h; ++i) zbuffer[i] = 0;
	...
	Vector3 camera = {0, 0, 1};
		...
		int y3 = p3.y;
		int lx, hx, ly, hy;
		if (olivec_normalize_triangle(canvas.width, canvas.height, x1, y1, x2, y2, x3, y3, &lx, &hx, &ly, &hy)) {
		    for (int y = ly; y <= hy; ++y) {
			for (int x = lx; x <= hx; ++x) {
			    int u1, u2, det;
				...
				if (1.0f / far_plane < z && z < 1.0f / near_plane && z > zbuffer[y * win->w + x]) {
				    zbuffer[y * win->w + x] = z;
				    OLIVEC_PIXEL(canvas, x, y) = mix_colors3(0xFF1818FF, 0xFF18FF18, 0xFFFF1818, u1, u2, det);
					...
					uint32_t v = z*255;
					if (v > 255) v = 255;
					olivec_blend_color(&OLIVEC_PIXEL(canvas, x, y), (v<<(3*8)));
				    }
				}
			    }
			}
